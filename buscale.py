...
def app():
    ...
    st.title("Text to analixer - Contabilidad 📝")
    ...
    if submit_button:
        ...
        resultado_db = clasificar_texto(text_query)

        if resultado_db.get("coincidencia", 0) > 0:
            st.success(f"Tema detectado: *{resultado_db['tema']}* con {resultado_db['coincidencia']}% de coincidencia.")
            st.write("Palabras encontradas:", ", ".join(resultado_db["palabras_detectadas"]))
        else:
